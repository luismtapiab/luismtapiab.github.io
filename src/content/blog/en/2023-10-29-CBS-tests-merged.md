...
The tools enable to the designer to generate a working interpreter based on a compact specification. Since the parts that make a language could be also in others this approach encourages reusability. The workflow (showed above) describes four stages of program generation every stage uses the tools generated in the previous one. The focus for the designer is on the last two, writing the specification that generates a translator to funcons and using that translator followed by the funcons interpreter to finally executing the program.
